...
| M9 | source: ci 且流水线在 timeout_s 内无终态 → defect 超时 → degraded | talos-ci-short-timeout | 是 | 用停 runner 触发超时；45s 短超时在流水线 10s 完成的环境下无法触发。`require_push: true, source: ci, credentials: gitlab`；body 首行 `repo: {PILOT_REPO}` ✅已补 |
