...
from tests.acceptance._config import (
    GITLAB_URL, PILOT_REPO, PILOT_PID, RESULTS_FILE, KANBAN_DB, TALOS_HOME, REPO_DIR,
    HERMES_HOME, HERMES_VENV_PYTHON, HERMES_AGENT_SRC,
)

# PILOT_ID = PILOT_PID (GitLab API accepts both numeric ID and URL-encoded path)
PILOT_ID = PILOT_PID

EXECUTOR_LOG = TALOS_HOME / "executor.jsonl" if isinstance(TALOS_HOME, Path) else Path(str(TALOS_HOME)) / "executor.jsonl"
GITLAB_TOKEN = os.environ.get("TALOS_GITLAB_ADMIN_TOKEN", "")
COMMIT = subprocess.run(["git", "rev-parse", "--short", "HEAD"],
    capture_output=True, text=True,
    cwd=str(REPO_DIR)).stdout.strip()
EXECUTOR_PID = int(os.environ.get("TALOS_EXECUTOR_PID", "0"))
...
def test_M1():
    ...
    env.update({
        ...
        "PYTHONPATH": f"{REPO_DIR}:{HERMES_AGENT_SRC}",
        "HERMES_HOME": str(HERMES_HOME),
    })
    proc = subprocess.Popen(
        [HERMES_VENV_PYTHON, "-m", "talos.executor.main"],
        env=env,
...
def test_M18():
    ...
    loop_path = str(REPO_DIR / "talos" / "executor" / "loop.py")
    ...
    sentinel_path = str(REPO_DIR / "talos" / "executor" / "sentinel.py")
    ...
    reap_path = str(REPO_DIR / "talos" / "executor" / "reap.py")
...
def test_M19():
    ...
    if isinstance(tokens, list):
        ...
        creds_path = str(REPO_DIR / "talos" / "executor" / "credentials.py")
...
def test_M22():
    ...
    executor_dir = REPO_DIR / "talos" / "executor"
    violations = []
...
def test_A2():
    ...
    adj_path = str(REPO_DIR / "talos" / "executor" / "adjudicate.py")
