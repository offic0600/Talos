...
def _self_check(
    ...
    """Startup self-check (§11, v2.1 #9, v2.2 #4, v2.3 §11 warn-only).

    8 items total (6 hard, 2 warn):
      (1)   ``hermes_cli`` importable — HARD FAIL
      (1b)  ``TALOS_ADJ_SLEEP`` debug hook — WARN
      (2)   ``WORKER_IMAGE`` exists in ``docker images`` — HARD FAIL
      (3)   Kanban DB path readable — HARD FAIL
      (3b)  Shadow DB warning — WARN
      (4)   GitLab reachable — WARN (v2.3 §11: connection failures warn-only;
            HTTP 401/403 remains hard fail for token misconfiguration)
      (5)   Skills installed and match repo (md5 check) — HARD FAIL
      (6)   Container config renderable — HARD FAIL
    ...
    try:
        ...
        print("[talos-executor] self-check (1/8) hermes_cli importable: OK")
    except ImportError as e:
        print(f"[talos-executor] self-check (1/8) hermes_cli importable: FAIL — {e}")
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"[talos-executor] self-check (2/8) docker image '{worker_image}': OK")
        else:
            print(
                f"[talos-executor] self-check (2/8) docker image '{worker_image}': FAIL — "
                "image not found"
            )
            print(
        ...
        print("[talos-executor] self-check (2/8) docker image: FAIL — docker not found")
        sys.exit(1)
    except subprocess.TimeoutExpired:
        print("[talos-executor] self-check (2/8) docker image: FAIL — docker inspect timed out")
        sys.exit(1)
    ...
    if db_path.is_file() and os.access(str(db_path), os.R_OK):
        print(f"[talos-executor] self-check (3/8) kanban DB readable: OK ({db_path})")
    else:
        print(
            f"[talos-executor] self-check (3/8) kanban DB readable: FAIL — "
    ...
    # ── (4) GitLab reachable — WARN ONLY (v2.3 §11) ─────────────────
    # DD v2.3 §11: "GitLab 可达性（不可达只告警）"
    # Connection-layer failures (DNS, timeout, TLS, refused, HTTP 5xx) → WARN.
    # HTTP 401/403 → HARD FAIL (token misconfiguration, won't self-heal).
    ...
    try:
        ...
        print(f"[talos-executor] self-check (4/8) GitLab reachable: OK ({gitlab_url})")
    except urllib.error.HTTPError as e:
        if e.code in (401, 403):
            # Token misconfiguration — won't self-heal, hard fail
            print(
                f"[talos-executor] self-check (4/8) GitLab reachable: FAIL — "
                f"HTTP {e.code} {e.reason}"
            )
            print("[talos-executor] hint: check TALOS_GITLAB_ADMIN_TOKEN in ~/.hermes/talos.env")
            sys.exit(1)
        else:
            # HTTP 5xx and other HTTP errors — server-side, may self-heal
            msg = f"GitLab returned HTTP {e.code} {e.reason} ({gitlab_url})"
            print(f"[talos-executor] self-check (4/8) GitLab reachable: WARN — {msg}")
            log_event("warn", msg=f"self-check GitLab: {msg}")
    except Exception as e:
        # Connection-layer failure (DNS, timeout, TLS, refused) — warn only
        msg = f"GitLab not reachable: {e} ({gitlab_url})"
        print(f"[talos-executor] self-check (4/8) GitLab reachable: WARN — {msg}")
    ...
    if not skill_dirs:
        print(f"[talos-executor] self-check (5/8) skills: FAIL — no skills found in {repo_skills}")
        sys.exit(1)
    ...
    for sname in skill_dirs:
        ...
        if not home_md.is_file():
            print(f"[talos-executor] self-check (5/8) skill '{sname}': FAIL — not installed in {home_skills}")
            all_ok = False
            continue
        ...
        if repo_hash != home_hash:
            print(f"[talos-executor] self-check (5/8) skill '{sname}': FAIL — md5 mismatch (repo={repo_hash[:8]} home={home_hash[:8]})")
            all_ok = False
        else:
            print(f"[talos-executor] self-check (5/8) skill '{sname}': OK (md5 {repo_hash[:8]})")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if not (has_model and has_provider and has_base_url):
                print(
                    f"[talos-executor] self-check (6/8) config render: FAIL — "
            ...
            print(
                f"[talos-executor] self-check (6/8) config render: OK "
                f"(model={model}, provider={provider}, base_url={base_url})"
            )
    except RuntimeError as e:
        print(f"[talos-executor] self-check (6/8) config render: FAIL — {e}")
...
def run_executor(
    *,
    ...
    # ── Deploy self-check (§11, v2.1 #9, v2.3 §11) ─────────────────
    # Hard-fail items: hermes_cli importable, WORKER_IMAGE exists in
    # docker images, kanban DB path readable, skills match, config renders.
    # Warn-only items: GitLab reachable (connection failures), TALOS_ADJ_SLEEP, shadow DB.
    # HTTP 401/403 for GitLab remains hard fail (token misconfiguration).
